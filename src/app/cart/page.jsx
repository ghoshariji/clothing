...
//         {
//           id: 1,
//           name: "APPLE iPhone 15 5G, 256GB, Gold",
//           price: 2997,
//           quantity: 3,
...
//                           </svg>
//                         </button>
//                         <input type="text" className="w-10 text-center text-sm font-medium text-gray-900 dark:text-white" value={item.quantity} readOnly />
//                         <button type="button" className="counter-button">
...
const Cart = () => {
  ...
  const totalPrice = cartItems.reduce((total, item) => total + (item.quantity * item.discountPrice), 0);
    ...
    <>
      ...
      <section className="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
        <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
                  ...
                  cartItems.map((item) => (
                    <div key={item._id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
                      <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                        <div className="flex items-center justify-between md:order-3 md:justify-end">
                          <div className="flex items-center">
                            <button type="button" className="counter-button" onClick={() => decreaseQuantity(item._id)}>
                              <svg className="h-2.5 w-2.5 text-gray-900 dark:text-white" fill="none" viewBox="0 0 18 2">
                                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 1h16" />
                              </svg>
                            </button>
                            <input type="text" className="w-10 text-center text-sm font-medium text-gray-900 dark:text-white" value={item.quantity} readOnly />
                            <button type="button" className="counter-button" onClick={() => increaseQuantity(item._id)}>
                              <svg className="h-2.5 w-2.5 text-gray-900 dark:text-white" fill="none" viewBox="0 0 18 18">
                                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 1v16M1 9h16" />
                              </svg>
                            </button>
                          ...
                          <div className="text-end md:w-32">
                            <p className="text-base font-bold text-gray-900 dark:text-white">Rs. {item.discountPrice}</p>
                          </div>
                        </div>

                        <a href="#" className="w-20 shrink-0 md:order-1">
                          <img className="h-20 w-20 dark:hidden" src={item.image} alt={item.productName} />
                          <img className="hidden h-20 w-20 dark:block" src={item.image} alt={item.productName} />
                        </a>

                        <div className="w-full flex-1 space-y-4 md:order-2 md:max-w-md">
                          <a href="#" className="text-base font-medium text-gray-900 hover:underline dark:text-white">{item.productName}</a>
                          <div className="flex items-center gap-4">
                            <button type="button" className="inline-flex items-center text-sm font-medium text-red-600 hover:underline dark:text-red-500" onClick={() => removeFromCart(item._id)}>
                              <svg className="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L17.94 6M18 18L6.06 6" />
                              </svg>
                              Remove
          ...
          {/* Button for total price */}
          <div className="mt-6 flex justify-center">
            <button className="w-full max-w-xs bg-blue-600 text-white py-3 rounded-lg shadow-lg hover:bg-blue-700 focus:outline-none">
